...
export async function POST(req: NextRequest) {
  try {
    ...
    if (adminCount === 0) {
      await AdminModel.create({ email: emailLower, password });
    }
    const user = await AdminModel.findOne({ email: emailLower }).select(
      "+password"
    );
    ...
    const userWithoutPassword = await AdminModel.findById(user._id).select(
      "-password"
    );
